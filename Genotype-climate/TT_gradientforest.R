...
write.csv(tt_gv_0085, "tt_genomic_offset_2100_RCP85.csv")

#Convert points of average value to raster
tt_gv_0085_raster <- rasterFromXYZ(tt_gv_0085)

pdf("tt_offset_0085.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tt_gv_0085_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_0026) <- colnames(tt_current_bio)
...
## this for loop takes long time
tt_gv_0026 <- cbind(fut_0026[,c("x","y")], df)
write.csv(tt_gv_0026, "tt_genomic_offset_2100_RCP26.csv")

#Convert points of average value to raster
tt_gv_0026_raster <- rasterFromXYZ(tt_gv_0026)

pdf("tt_offset_0026.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tt_gv_0026_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_5026) <- colnames(tt_current_bio)
...
## this for loop takes long time
tt_gv_5026 <- cbind(fut_5026[,c("x","y")], df)
write.csv(tt_gv_5026, "tt_genomic_offset_2050_RCP26.csv")

#Convert points of average value to raster
tt_gv_5026_raster <- rasterFromXYZ(tt_gv_5026)

pdf("tt_offset_5026.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tt_gv_5026_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_5085) <- colnames(tt_current_bio)
...
## this for loop takes long time
tt_gv_5085 <- cbind(fut_5085[,c("x","y")], df)
write.csv(tt_gv_5085, "tt_genomic_offset_2050_RCP85.csv")

#Convert points of average value to raster
tt_gv_5085_raster <- rasterFromXYZ(tt_gv_5085)

pdf("tt_offset_5085.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tt_gv_5085_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()