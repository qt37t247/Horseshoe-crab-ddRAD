...
tg_snp <- read.csv("tg.alfreq", row.names = 1)
#Load bio-factor
tg_env <- read.csv("tg_env.csv", header = TRUE)
tg_env <- tg_env[, -1]
rownames(tg_env) <- rownames(tg_snp)#Match row names
#GF analysis
nSites <- dim(tg_snp)[1]
nSpecs <- dim(tg_snp)[2]
lev <- floor(log2(nSites*0.368/2))
lev
gf <- gradientForest(cbind(tg_env,tg_snp), predictor.vars=colnames(tg_env), 
                     response.vars=colnames(tg_snp), ntree = 500, maxLevel = lev, 
                     corr.threshold = 0.5, compact = T, nbin = 201)
gf
##Important variables:
##[1] BO21_salinityrange_ss BO21_chlorange_ss     BO21_temprange_ss     BO21_temprange_bdmin 
##[5] BO21_tempmean_ss  
...
tg_current_bio <- read.csv("current_bio.csv")
tg_current_bio <- tg_current_bio[,-1][complete.cases(tg_current_bio[,-1]), ]
imp.vars <- c("BO21_salinityrange_ss","BO21_chlorange_ss","BO21_temprange_ss","BO21_temprange_bdmin","BO21_tempmean_ss")
Trns_grid <- cbind(tg_current_bio[,c("x","y")], predict(gf,tg_current_bio[,imp.vars]))
PCs <- prcomp(Trns_grid[,imp.vars])
sgn <- sign(PCs$rotation["BO21_salinityrange_ss",])
...
lo <- which(l.y < 0) # Get the variables on the botgom half of the plot
...
colnames(fut_0085) <- colnames(tg_current_bio)
...
tg_gv_0085 <- cbind(fut_0085[,c("x","y")], df)
write.csv(tg_gv_0085, "tg_genomic_offset_2100_RCP85.csv")

#Convert points of average value to raster
tg_gv_0085_raster <- rasterFromXYZ(tg_gv_0085)

pdf("tg_offset_0085.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tg_gv_0085_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_0026) <- colnames(tg_current_bio)
...
## this for loop takes long time
tg_gv_0026 <- cbind(fut_0026[,c("x","y")], df)
write.csv(tg_gv_0026, "tg_genomic_offset_2100_RCP26.csv")

#Convert points of average value to raster
tg_gv_0026_raster <- rasterFromXYZ(tg_gv_0026)

pdf("tg_offset_0026.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tg_gv_0026_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_5026) <- colnames(tg_current_bio)
...
## this for loop takes long time
tg_gv_5026 <- cbind(fut_5026[,c("x","y")], df)
write.csv(tg_gv_5026, "tg_genomic_offset_2050_RCP26.csv")

#Convert points of average value to raster
tg_gv_5026_raster <- rasterFromXYZ(tg_gv_5026)

pdf("tg_offset_5026.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tg_gv_5026_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()
...
colnames(fut_5085) <- colnames(tg_current_bio)
...
## this for loop takes long time
tg_gv_5085 <- cbind(fut_5085[,c("x","y")], df)
write.csv(tg_gv_5085, "tg_genomic_offset_2050_RCP85.csv")

#Convert points of average value to raster
tg_gv_5085_raster <- rasterFromXYZ(tg_gv_5085)

pdf("tg_offset_5085.pdf")
pal <- colorRampPalette(c("yellow","red"))
plot(tg_gv_5085_raster, col = pal(15), breaks = seq(0, 0.05, length.out=16))
dev.off()