...
M.gen.diss <- read.table(file = 'TT0L_gdist.txt', sep = '\t', header = T, row.names = 1, fill = T)
M.gen.diss <- as.dist(M.gen.diss, diag = TRUE, upper = TRUE)
#load geographical coordinates and create geo distance matrix
sample.points <- read.table(file = 'TT.coordinates', sep = '\t', header = T, row.names = 1, fill = T)
names(sample.points) <- c('x', 'y')
M.geo.dist <- read.table(file = 'TT_dist.txt', sep = '\t', header = T, row.names = 1, fill = T)
...
###Plot IBD###
pdf("TT_IBD.pdf")
plot(IBD.data, pch=19)
abline(IBD.model, col="red")
dev.off()
...
#Output results
write.csv(midpoints, file ='TT_MidPoint.csv')
write.csv(grid, file = 'TT_DresD.csv')

##########Visualization##############
TT <- read.csv("TT_DresD.csv")
grid <- TT[, -1]
