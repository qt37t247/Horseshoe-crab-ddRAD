...
M.gen.diss <- read.table(file = 'TG0L_gdist.txt', sep = '\t', header = T, row.names = 1, fill = T)
M.gen.diss <- as.dist(M.gen.diss, diag = TRUE, upper = TRUE)
#load geographical coordinates and create geo distance matrix
sample.points <- read.table(file = 'TG.coordinates', sep = '\t', header = T, row.names = 1, fill = T)
names(sample.points) <- c('x', 'y')
M.geo.dist <- read.table(file = 'TG_dist.txt', sep = '\t', header = T, row.names = 1, fill = T)
...
#Output results
write.csv(midpoints, file ='TG_MidPoint.csv')
write.csv(grid, file = 'TG_DresD.csv')

##########Visualization##############
TG <- read.csv("TG_DresD.csv")
grid <- TG[, -1]
